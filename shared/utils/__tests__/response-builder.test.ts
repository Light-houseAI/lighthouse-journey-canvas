...
describe('Response Builder Utilities', () => {
  ...
  describe('createSuccessResponse', () => {
    ...
    it('should include request ID when provided', () => {
      const response = createSuccessResponse(testData, { requestId: testRequestId });
      
      expect(response.meta?.requestId).toBe(testRequestId);
    });
    
    it('should include additional metadata', () => {
      const customMeta = { count: 5, viewingUser: 'testuser' };
      const response = createSuccessResponse(testData, { meta: customMeta });
  ...
  describe('Edge cases', () => {
    ...
    it('should handle undefined in error details', () => {
      const response = createErrorResponse('Error', { details: undefined });
