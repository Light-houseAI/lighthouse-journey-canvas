/**
 * CareerTransitionNode - Dedicated Career Transition Node Component
 *
 * Specialized node component for career transition experiences with optimized layout
...
export const CareerTransitionNode: React.FC<NodeProps<CareerTransitionNodeData>> = ({
  data,
}) => {
  const {
    node,
  ...
  // CareerTransition-specific node styling with transition theme
  const getCareerTransitionNodeStyle = (): React.CSSProperties => {
    const baseColor = '#8b5cf6'; // Violet for career progression and transformation
    const accentColor = '#7c3aed'; // Deeper violet
    const isHighlighted = isSelected || isFocused;

    return {
      // Transformative gradient with career transition-specific colors
      ...
      borderRadius: '20px',
      padding: '0px',
      width: '240px', // Slightly wider for career transition info
      height: '180px', // Taller for transition title + description info
  ...
  // Extract career transition-specific data
  const title = node.meta?.title || node.title || 'Career Transition';
  const description = node.meta?.description || '';

  return (
    <div
      ref={nodeRef}
      style={getCareerTransitionNodeStyle()}
      onClick={handleNodeClick}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
      className="career-transition-node group relative transform hover:scale-[1.02] transition-transform duration-300"
    >
      {/* Transformative background effects */}
      ...
      <div className="relative z-10 h-full w-full flex flex-col justify-between px-3">
        ...
        {/* Row 2: Career Transition Information */}
      ...
      {/* Career Transition-specific Hover Tooltip */}
      {isHovered && createPortal(
        ...
        >
          {/* Enhanced Career Transition Tooltip */}
          <div
            style={{
              ...getCareerTransitionNodeStyle(),
              width: '360px',
              height: '240px',
          ...
          >
            ...
            <div className="relative z-10 h-full w-full flex flex-col justify-between px-4">
              ...
              {/* Row 2: Enhanced Career Transition Information */}
...
export default CareerTransitionNode;