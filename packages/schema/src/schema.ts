...
// User peer-insights preferences (receive/share toggles)
export const userPreferences = pgTable('user_preferences', {
  ...
  userId: integer('user_id')
    ...
    .unique()
    .references(() => users.id, { onDelete: 'cascade' }),
  receivePeerInsights: boolean('receive_peer_insights').notNull().default(false),
  sharePeerInsights: boolean('share_peer_insights').notNull().default(false),
  shareScopeDefault: varchar('share_scope_default', { length: 20 })
    .notNull()
    .default('all'),
...
export const sessionMappings = pgTable('session_mappings', {
  ...
  peerInsights: jsonb('peer_insights'),
