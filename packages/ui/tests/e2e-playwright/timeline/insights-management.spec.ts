...
test.describe.skip(
  'Timeline Node Insights Management - SKIPPED: ReactFlow removed, needs update for ProfileListView',
  () => {
    let testUser: {
      email: string;
      password: string;
    ...
    test('complete insights CRUD workflow', async ({ page }) => {
      ...
      await test.step('Navigate to timeline and select node', async () => {
        ...
        if (await timelineNode.isVisible({ timeout: 3000 })) {
          ...
          const specificNode = page.getByTestId(
            'rf__node-97f9cb11-512f-496e-84b4-8defe5131df4'
          );
      ...
      await test.step('Create new insight', async () => {
        ...
        const addInsightButton = page.getByRole('button', {
          name: 'Add Insight',
        });
        ...
        const insightTextbox = page.getByRole('textbox', {
          name: 'Share your insight *',
        });
        ...
        const referenceTextbox = page.getByRole('textbox', {
          name: 'URL, book reference, note,',
        });
        ...
        const insightDisplay = page.getByText(
          'Key Lessons from This Experiencejust nowTesting new insightShow more'
        );
      ...
      await test.step('Edit existing insight', async () => {
        ...
        if (await contextMenu.isVisible({ timeout: 2000 })) {
          ...
          const referenceText = page
            .getByLabel('Edit Insight')
            .getByText('new book');
    ...
    test('node editing and insight interaction', async ({ page }) => {
      ...
      await test.step('Edit timeline node', async () => {
        ...
        const educationNode = page.getByText(
          'bachelor of technology, bachelorsDurationJan 2004 - Jan 2008Studybachelor of'
        );
      ...
      await test.step('Test insights form accessibility', async () => {
        ...
        if (await addInsightButton.isVisible({ timeout: 3000 })) {
          ...
          if (await insightTextbox.isVisible({ timeout: 2000 })) {
            ...
            const referenceField = page.getByRole('textbox', {
              name: 'URL, book reference, note,',
            });
        ...
        if (await addInsightButton.isVisible({ timeout: 3000 })) {
          ...
          if (await saveButton.isVisible({ timeout: 2000 })) {
            ...
            const validationError = await page
              .locator('text=required, text=cannot be empty')
            ...
            if (validationError) {
              ...
              console.log(
                'ℹ️ No validation message detected - may use different validation approach'
              );
            }
          }
        }
