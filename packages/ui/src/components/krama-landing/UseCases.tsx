...
const IconSvg = ({ icon, color }: { icon: string; color: string }) => {
  ...
  return icons[icon] || <span className="text-lg font-bold" style={{ color }}>{icon[0].toUpperCase()}</span>;
};
            ...
            <div className="flex gap-2 mb-8">
              {useCases.map((uc) => (
                <div
                  ...
                  style={{
                    backgroundColor: uc.id === activeCase
                      ? 'hsl(var(--krama-primary))'
                      : 'hsl(var(--krama-border))',
                  }}
                />
              ))}
            </div>
                  ...
                  <div className="bg-krama-card border border-krama-border rounded-xl shadow-lg p-4 min-w-[280px]">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div
                        ...
                        style={{ backgroundColor: 'hsl(var(--krama-muted))' }}
                      >
                    ...
                    <div
                      className="flex items-center justify-between rounded-lg p-3 relative"
                      style={{ backgroundColor: 'hsla(var(--krama-muted), 0.5)' }}
                    >
                      <div>
                        <p className="text-sm font-medium text-krama-primary">Krama</p>
                        <p className="text-xs text-krama-text-body">
                          Last worked: 1 hour ago{' '}
                          <span className="text-krama-accent font-medium">2 sessions to review</span>
                        </p>
                      </div>

                      <div className="relative">
                        <button
                          className={`text-xs px-4 py-2 rounded-md font-medium flex items-center gap-1 transition-all duration-200 text-white`}
                          style={{
                            backgroundColor: animationPhase === 'clicked'
                              ? 'hsl(var(--krama-primary))'
                              : 'hsl(var(--krama-foreground))',
                            transform: animationPhase === 'clicking' ? 'scale(0.95)' : 'scale(1)',
                            opacity: animationPhase === 'clicking' ? 0.8 : 1,
                          }}
                        >
                          Start session
                        ...
                        >
                          <MousePointer2
                            className={`w-5 h-5 text-krama-primary transition-transform ${
                              animationPhase === 'clicking' ? 'rotate-[-10deg]' : ''
                            }`}
                            style={{ fill: 'hsla(var(--krama-primary), 0.2)' }}
                          />
                        </div>
                      </div>
                    </div>
