    ...
    <div className="absolute inset-0 flex items-center justify-center gap-12">
        ...
        {inputSources.map((source, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs text-krama-primary font-medium whitespace-nowrap">{source.label}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="flex flex-col items-center">
        <div
          ...
          style={{
            backgroundColor: phase === 'processing' || phase === 'output'
              ? 'hsl(var(--krama-primary))'
              : 'hsl(var(--krama-muted))',
            transform: phase === 'processing' ? 'scale(1.1)' : 'scale(1)',
          }}
        >
          {phase === 'processing' && (
            <div
              className="absolute inset-0 rounded-full animate-ping"
              style={{ border: '2px solid hsl(var(--krama-primary))', animationDuration: '1.5s' }}
            />
          )}
          <Activity
            className="w-7 h-7 transition-colors duration-300"
            style={{
              color: phase === 'processing' || phase === 'output' ? 'white' : 'hsl(var(--krama-muted-foreground))',
            }}
            strokeWidth={1.5}
          />
        </div>
      ...
      <div
        className={`w-64 rounded-xl overflow-hidden shadow-lg transition-all duration-700 ${
          phase === 'output' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'
        }`}
        style={{ backgroundColor: 'hsl(var(--krama-primary))' }}
      >
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: 'hsla(var(--krama-primary), 0.2)' }}
                >
                  <Check className="w-2.5 h-2.5 text-krama-accent" strokeWidth={3} />
                </div>
                <div>
                  <p className="text-xs font-medium text-krama-primary">{item.title}</p>
                  <p className="text-[10px] text-krama-muted-foreground leading-relaxed">{item.description}</p>
                </div>
              </div>
            );
          })}
