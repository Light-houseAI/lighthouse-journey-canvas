...
const ExecuteAnimation = ({ isActive }: ExecuteAnimationProps) => {
  const [phase, setPhase] = useState<'idle' | 'manual' | 'automating' | 'running' | 'improving'>(
    'idle',
  );
  ...
  return (
    <div className="relative w-full h-full min-h-[400px] flex flex-col p-8 overflow-hidden bg-krama-background">
      {/* Title */}
      <div className="mb-6">
        <h3 className="text-2xl font-bold mb-2 text-krama-primary">Execute</h3>
              ...
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    boxShadow: checked
                      ? '0 0 12px hsla(var(--krama-primary) / 0.4)'
                      : 'none',
                  }}
                >
                  <Check
                ...
                <span
                  className={`flex-1 font-medium transition-all duration-500 ${
                    automated ? 'opacity-100 text-krama-foreground' : 'opacity-60 text-krama-muted-foreground'
                  }`}
                >
                  {task.text}
                </span>
                ...
                <div
                  className={`
                    px-2 py-1 rounded text-xs font-semibold text-krama-primary
                    transition-all duration-300 ease-out
                    ${automated ? 'opacity-100 translate-x-0 bg-krama-primary/15' : 'opacity-0 translate-x-4 bg-transparent'}
                  `}
                >
                  auto
                </div>
      ...
      <div
        className={`text-center transition-all duration-500 ${
          phase === 'running' || phase === 'improving'
            ? 'opacity-100 translate-y-0'
            : 'opacity-0 translate-y-4'
        }`}
      >
        <p className="text-lg font-semibold mb-2 text-krama-primary">Running automatically</p>
      ...
      >
        ...
        />
        <span className="text-sm text-krama-muted-foreground">Keep improving</span>
      </div>
    </div>
  );
};
