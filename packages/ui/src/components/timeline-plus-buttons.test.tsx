...
import { Timeline, TimelineConfig, TimelineNode } from './timeline/Timeline';
...
describe('Timeline Plus Buttons', () => {
  ...
  test('adds start and end plus buttons for root timeline', () => {
    ...
    expect(
      screen.getByTestId('edge-timeline-start-plus-level-0')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('edge-timeline-end-plus-level-0')
    ).toBeInTheDocument();

    // Should use dotted timeline edge type
    expect(
      screen.getByTestId('edge-timeline-start-plus-level-0')
    ).toHaveAttribute('data-edge-type', 'dottedTimeline');
    expect(
      screen.getByTestId('edge-timeline-end-plus-level-0')
    ).toHaveAttribute('data-edge-type', 'dottedTimeline');
  });
  ...
  test('adds only end plus button for child timeline', () => {
    ...
    expect(
      screen.queryByTestId('edge-timeline-start-plus-level-1-parent-parent-1')
    ).not.toBeInTheDocument();
    expect(
      screen.getByTestId('edge-timeline-end-plus-level-1-parent-parent-1')
    ).toBeInTheDocument();
  ...
  test('start plus button triggers onPlusButtonClick with before insertion point', async () => {
    ...
    const startPlusButton = screen.getByTestId(
      'timeline-end-plus-button-timeline-start-plus-level-0'
    );
  ...
  test('end plus button triggers onPlusButtonClick with after insertion point', async () => {
    ...
    const endPlusButton = screen.getByTestId(
      'timeline-end-plus-button-timeline-end-plus-level-0'
    );
  ...
  test('child timeline end plus button works correctly', async () => {
    ...
    const childEndPlusButton = screen.getByTestId(
      'timeline-end-plus-button-timeline-end-plus-level-1-parent-parent-1'
    );
  ...
  test('plus buttons connect to helper nodes correctly', () => {
    ...
    expect(
      screen.getByTestId('node-timeline-start-helper-level-0')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('node-timeline-end-helper-level-0')
    ).toBeInTheDocument();

    // Start edge should connect helper to first node
    expect(
      screen.getByTestId('edge-timeline-start-plus-level-0')
    ).toHaveAttribute('data-source', 'timeline-start-helper-level-0');
    expect(
      screen.getByTestId('edge-timeline-start-plus-level-0')
    ).toHaveAttribute('data-target', 'node-1');

    // End edge should connect last node to helper
    expect(
      screen.getByTestId('edge-timeline-end-plus-level-0')
    ).toHaveAttribute('data-source', 'node-1');
    expect(
      screen.getByTestId('edge-timeline-end-plus-level-0')
    ).toHaveAttribute('data-target', 'timeline-end-helper-level-0');
  });
  ...
  test('multiple timelines have independent plus buttons', () => {
    ...
    expect(
      screen.queryByTestId('edge-timeline-start-plus-level-1-parent-parent-2')
    ).not.toBeInTheDocument();
    expect(
      screen.getByTestId('edge-timeline-end-plus-level-1-parent-parent-1')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('edge-timeline-end-plus-level-1-parent-parent-2')
    ).toBeInTheDocument();
  });

  test('adds plus buttons below leaf nodes (nodes without children)', () => {
    const leafNode1 = createMockTimelineNode({
      id: 'leaf-1',
      start: '2022-01',
    });
    const leafNode2 = createMockTimelineNode({
      id: 'leaf-2',
      start: '2023-01',
    });
    ...
    // Should use dotted timeline edge type
    expect(screen.getByTestId('edge-leaf-plus-leaf-1')).toHaveAttribute(
      'data-edge-type',
      'dottedTimeline'
    );
    expect(screen.getByTestId('edge-leaf-plus-leaf-2')).toHaveAttribute(
      'data-edge-type',
      'dottedTimeline'
    );
  ...
  test('leaf node plus button triggers onPlusButtonClick with child insertion point', async () => {
    ...
    const leafPlusButton = screen.getByTestId(
      'timeline-end-plus-button-leaf-plus-leaf-node'
    );
  ...
  test('does not add leaf plus buttons to nodes with children', () => {
    ...
    expect(
      screen.queryByTestId('edge-leaf-plus-parent-with-children')
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId('node-leaf-helper-parent-with-children')
    ).not.toBeInTheDocument();
