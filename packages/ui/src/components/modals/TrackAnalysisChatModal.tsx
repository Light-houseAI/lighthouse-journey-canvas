 ...
 * Interactive chat interface for analyzing track activity and getting insights
 *
...
class ChatModalErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('[TrackAnalysisChatModal] Error caught:', error, errorInfo);
  }
...
interface TrackAnalysisChatModalProps {
  isOpen: boolean;
  onClose: () => void;
  nodeId: string;
}

function TrackAnalysisChatModalContent({
  onClose,
  nodeId,
}: Omit<TrackAnalysisChatModalProps, 'isOpen'>) {
  // Initialize with welcome message
  const getInitialMessage = (): Message => ({
    id: '1',
    type: 'ai',
    content: `Let's analyze your **track activity**. I'll review your recent work sessions to provide insights, summaries, and actionable recommendations.\n\nYou can ask me questions like:\n- "What did I work on this week?"\n- "Summarize my recent progress"\n- "What are my key accomplishments?"\n- "Help me prepare a status update"`,
    timestamp: new Date(),
  });
  ...
  useEffect(() => {
    const sessions = getChatSessions(nodeId, 'track-analysis');
      ...
      if (currentSession && currentSession.title === 'New conversation') {
        ...
        if (userMessages.length > 0) {
          ...
          generateSessionTitleWithLLM(currentSession).catch((error) => {
            console.error('[TrackAnalysisChatModal] Failed to generate title on close:', error);
          });
        }
      }
    };
  ...
  const initializeSession = useCallback(() => {
    ...
    const session = createChatSession(nodeId, 'track-analysis', [
      {
        id: initialMsg.id,
    ...
    setPastSessions(getChatSessions(nodeId, 'track-analysis'));
  }, [nodeId]);
  ...
  const {
    ...
    onError: (err: Error) => {
      const errorMessage: Message = {
        id: Date.now().toString(),
        type: 'ai',
        content: `I encountered an issue while analyzing your track: ${err.message}. Please try again or rephrase your question.`,
        timestamp: new Date(),
      };
  ...
  const handleLoadPastSession = (sessionId: string) => {
    const session = getChatSession(nodeId, 'track-analysis', sessionId);
    if (session) {
      setCurrentSession(session);
  ...
  const handleDeleteSession = (sessionId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    deleteChatSession(nodeId, 'track-analysis', sessionId);
    setPastSessions(getChatSessions(nodeId, 'track-analysis'));
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (!session) {
      const initialMsg = getInitialMessage();
      session = createChatSession(nodeId, 'track-analysis', [
        {
          id: initialMsg.id,
          type: initialMsg.type,
          content: initialMsg.content,
