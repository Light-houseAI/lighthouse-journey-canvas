/**
 * Weekly Progress Update Chat Modal
 * Interactive chat interface for creating/refining weekly progress updates with AI
 *
...
class ChatModalErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('[WeeklyProgressChatModal] Error caught:', error, errorInfo);
  }
...
interface WeeklyProgressChatModalProps {
  isOpen: boolean;
  onClose: () => void;
  nodeId: string;
}
...
function getCurrentWeekRange(): string {
  ...
  return `${formatDate(startOfWeek)} - ${endOfWeek.getDate()}, ${endOfWeek.getFullYear()}`;
}

function WeeklyProgressChatModalContent({
  onClose,
  nodeId,
}: Omit<WeeklyProgressChatModalProps, 'isOpen'>) {
  // Initialize with welcome message
  const getInitialMessage = (): Message => ({
    id: '1',
    type: 'ai',
    content: `Let's get started putting together your **Weekly progress update** for ${getCurrentWeekRange()}.\n\nYou can ask me questions like:\n- "What did I work on this week?"\n- "Summarize my coding sessions"\n- "What meetings did I have?"\n- "Generate my weekly report"`,
    timestamp: new Date(),
  });
  ...
  useEffect(() => {
    const sessions = getChatSessions(nodeId, 'weekly-progress');
      ...
      if (currentSession && currentSession.title === 'New conversation') {
        ...
        if (userMessages.length > 0) {
          ...
          generateSessionTitleWithLLM(currentSession).catch((error) => {
            console.error('[WeeklyProgressChatModal] Failed to generate title on close:', error);
          });
        }
      }
    };
  ...
  const initializeSession = useCallback(() => {
    ...
    const session = createChatSession(nodeId, 'weekly-progress', [
      {
        id: initialMsg.id,
    ...
    setPastSessions(getChatSessions(nodeId, 'weekly-progress'));
  }, [nodeId]);
  ...
  const {
    ...
    nodeId,
    lookbackDays: 7, // Focus on the past week for progress updates
    maxResults: 20,
    includeGraph: true,
    includeVectors: true,
    onSuccess: (result: NaturalLanguageQueryResult) => {
      ...
      // Show the report card when we have substantial context
      if (result.sources.length > 3 && result.confidence > 0.5) {
        setGeneratedReport({
    ...
    onError: (err: Error) => {
      const errorMessage: Message = {
        id: Date.now().toString(),
        type: 'ai',
        content: `I encountered an issue while generating your progress update: ${err.message}. Please try again or rephrase your question.`,
        timestamp: new Date(),
      };
  ...
  const handleLoadPastSession = (sessionId: string) => {
    const session = getChatSession(nodeId, 'weekly-progress', sessionId);
    if (session) {
      setCurrentSession(session);
  ...
  const handleDeleteSession = (sessionId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    deleteChatSession(nodeId, 'weekly-progress', sessionId);
    setPastSessions(getChatSessions(nodeId, 'weekly-progress'));
  ...
  const handleSendMessage = async () => {
    ...
    if (!session) {
      const initialMsg = getInitialMessage();
      session = createChatSession(nodeId, 'weekly-progress', [
        {
          id: initialMsg.id,
          type: initialMsg.type,
          content: initialMsg.content,
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `weekly-progress-${new Date().toISOString().split('T')[0]}.md`;
  ...
  // Generate markdown report from conversation
  const generateReportMarkdown = (): string => {
    const dateRange = getCurrentWeekRange();
    let markdown = `# Weekly Progress Update\n`;
    markdown += `**${dateRange}**\n\n`;
    markdown += `---\n\n`;

    // Add all AI responses as report sections
    const aiMessages = messages.filter(m => m.type === 'ai' && m.id !== '1'); // Exclude welcome message
    ...
    aiMessages.forEach((message, index) => {
      ...
      if (message.sources && message.sources.length > 0) {
        ...
        message.sources.forEach((source, idx) => {
          markdown += `${idx + 1}. **${source.title}** (${source.type})`;
          if (source.description) {
            markdown += ` - ${source.description.slice(0, 100)}...`;
          }
          markdown += `\n`;
        });
        markdown += `\n`;
      ...
      if (message.confidence !== undefined) {
        markdown += `*Confidence: ${Math.round(message.confidence * 100)}%*\n\n`;
      }

      markdown += `---\n\n`;
    });

    markdown += `\n*Generated by Lighthouse AI on ${new Date().toLocaleDateString()}*\n`;
          ...
          <div className="flex flex-1 flex-col overflow-hidden">
            ...
            >
              ...
              {generatedReport && (
                <div className="flex justify-start">
                  <div
                    className="w-[400px] rounded-2xl p-1"
                    style={{ background: '#D6D6D4' }}
                  >
                    <div
                    ...
                    >
                      ...
                      <button onClick={handleDownload}>
                        <Download className="h-6 w-6" style={{ color: '#000000' }} />
                      </button>
                    </div>
                  </div>
                </div>
