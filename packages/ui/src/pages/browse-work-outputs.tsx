...
import { ProgressSnapshotPanel } from '../components/timeline/ProgressSnapshotPanel';
import { WorkflowAnalysisPanel } from '../components/workflow/WorkflowAnalysisPanel';
...
const workOutputTemplates: WorkOutputTemplate[] = [
  ...
  },
  {
    id: 'case-study',
    title: 'Case study',
    description: 'A structured narrative of a project you worked on—covering the problem, approach, decisions, and outcomes.',
...
export default function BrowseWorkOutputsPage() {
  const [, params] = useRoute('/work-track/:nodeId/browse-outputs');
  ...
  const handleGetStarted = () => {
    if (selectedTemplate && selectedTemplate.isAvailable) {
      ...
      setLocation(`/work-track/${nodeId}?template=${selectedTemplate.templateKey}`);
    }
  };

  const handleRemixWithAI = () => {
    if (selectedTemplate?.templateKey === 'progress-snapshot') {
      setLocation(`/work-track/${nodeId}/browse-outputs/weekly-progress`);
    }
    if (selectedTemplate?.templateKey === 'workflow-analysis') {
      setLocation(`/work-track/${nodeId}/browse-outputs/workflow-analysis`);
    }
  };
  ...
  return (
    <div className="flex h-screen flex-col bg-white">
      ...
      >
        <button
          onClick={() => setLocation(`/work-track/${nodeId}`)}
        ...
        <div className="flex flex-1 flex-col gap-0.5">
          <h2
            ...
            style={{ color: '#161619', letterSpacing: '-0.05px', lineHeight: '30px' }}
          >
            Browse work session outputs
          </h2>
          <p
            className="text-base"
            style={{ color: '#161619', letterSpacing: '-0.05px', lineHeight: '24px' }}
          >
                  ...
                  <div key={category.id} className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        style={{ color: '#2E2E2E', letterSpacing: '-0.05px', lineHeight: '24px' }}
                      >
                        {category.title}
                      </span>
                    </div>
                  ...
                  <div className="flex items-start gap-2.5 p-4">
                    <div className="flex flex-1 flex-col gap-1.5">
                      <h3
                        ...
                        style={{ color: '#2E2E2E', letterSpacing: '-0.05px', lineHeight: '36px' }}
                      >
                      ...
                      {selectedTemplate.isAvailable && (
                        <div className="flex items-center gap-2">
                          <div
                            ...
                            style={{ background: 'linear-gradient(117.82deg, #9AC6B5 38.64%, #A897F2 85.27%)' }}
                          >
                            <img src={logoImage} alt="Lighthouse" className="h-3.5 w-3.5 object-contain" />
                          </div>
                          <span
                            className="text-sm"
                            style={{ color: '#4A4F4E', letterSpacing: '-0.05px', lineHeight: '22px' }}
                          >
                    ...
                    {selectedTemplate.isAvailable && (
                      <button
                        ...
                        style={{ boxShadow: '0px 10px 50px rgba(139, 139, 139, 0.1)' }}
                      >
                        <Heart
                          className="h-7 w-7"
                          fill={favoriteIds.has(selectedTemplate.id) ? '#EB3741' : 'none'}
                          style={{ color: favoriteIds.has(selectedTemplate.id) ? '#EB3741' : '#4A4F4E' }}
                        />
                      </button>
                    )}
                  </div>
                  ...
                  <div className="flex flex-1 flex-col items-center gap-6 px-4 pb-4">
                    ...
                    {selectedTemplate.isAvailable ? (
                      <>
                        ...
                        >
                          <div className="my-9 flex w-[595px] flex-col gap-7 rounded-lg bg-white px-[60px] py-12">
                            <div className="flex flex-col gap-2.5">
                              <div className="flex flex-col gap-2">
                                <span className="text-base font-semibold" style={{ color: '#212121', letterSpacing: '-0.05px' }}>
                                  Week of January 11 - 17
                                </span>
                                <h4 className="text-[28px] font-semibold" style={{ color: '#000000', letterSpacing: '-0.05px', lineHeight: '36px' }}>
                                  Weekly progress update
                                </h4>
                              </div>
                              <div className="flex w-fit items-center gap-2 rounded bg-[#FAFAFA] px-3 py-2">
                                <div className="h-4 w-4 rounded-full bg-gray-300" />
                                <span className="text-sm" style={{ color: '#000000' }}>Anna Amani</span>
                              </div>
                            </div>
                            <div className="flex flex-col gap-3">
                              <h5 className="text-base font-semibold" style={{ color: '#212121' }}>Summary</h5>
                              <p className="text-sm" style={{ color: '#212121', lineHeight: '22px' }}>
                                Focused on improving the checkout experience for small merchants by reducing friction in the authentication and confirmation steps.
                              </p>
                            </div>
                            <div className="flex flex-col gap-3">
                              <h5 className="text-base font-semibold" style={{ color: '#212121' }}>What I worked on</h5>
                      ...
                      <div className="flex w-full max-w-[700px] flex-col items-center gap-6 pt-[72px]">
                        <div
                          ...
                          style={{ background: '#F6F7F9', border: '1px solid #F1F2F9', boxShadow: '0px 3px 12px -2px rgba(170, 170, 190, 0.06)' }}
                        >
                          <div className="flex flex-1 items-start gap-4">
                            <div
                              className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-[9.67px]"
                              style={{ background: '#D4E7FF', boxShadow: 'inset 0px -1px 1px rgba(50, 50, 204, 0.13), inset 0px 1.19px 1.19px rgba(255, 255, 255, 0.35), inset 0px -4.17px 4.76px rgba(171, 182, 233, 0.3)' }}
                            >
                              <AlertOctagon className="h-6 w-6" style={{ color: '#033C84' }} />
                            </div>
                            <p className="flex-1 text-sm" style={{ color: '#4A4F4E', letterSpacing: '-0.05px', lineHeight: '22px' }}>
                          ...
                          <button
                            ...
                            style={{ borderColor: votedIds.has(selectedTemplate.id) ? '#3D6E63' : '#2E2E2E' }}
                          >
                            <ThumbsUp
                              className="h-5 w-5"
                              style={{ color: votedIds.has(selectedTemplate.id) ? '#3D6E63' : '#292D32' }}
                            ...
                            <span className="text-sm font-medium" style={{ color: '#292D32' }}>
                        ...
                        {feedbackSubmittedIds.has(selectedTemplate.id) ? (
                          <div className="flex w-full flex-col items-center gap-4 pt-8">
                            <img src={heartPreviewImage} alt="Feedback received" className="h-[225px] w-[300px] object-contain" />
                            <h4 className="text-xl font-semibold" style={{ color: '#1E1E1E', letterSpacing: '-0.05px', lineHeight: '30px' }}>
                              Feedback received
                            </h4>
                            <p className="text-base" style={{ color: '#2E2E2E', letterSpacing: '-0.05px', lineHeight: '24px' }}>
                          ...
                          <div className="flex w-full flex-col items-end gap-4">
                            <div className="flex w-full flex-col gap-3">
                              <h4 className="text-base font-semibold" style={{ color: '#2E2E2E', letterSpacing: '-0.05px', lineHeight: '24px' }}>
                              ...
                              <textarea
                                ...
                                style={{ borderColor: '#EAECF0', boxShadow: '3px 3px 10px rgba(120, 132, 149, 0.08)', color: '#2E2E2E', letterSpacing: '-0.05px', lineHeight: '24px' }}
                              />
                            </div>
                            <Button
                              ...
                              style={{ boxShadow: '0px 2px 5px rgba(103, 110, 118, 0.08), 0px 0px 0px 1px rgba(103, 110, 118, 0.16), 0px 1px 1px rgba(0, 0, 0, 0.12)' }}
                            >
                              <span className="text-sm font-semibold" style={{ color: '#000000' }}>Submit feedback</span>
                            </Button>
                          </div>
                        )}
                      </div>
                  ...
                  {selectedTemplate.isAvailable && (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-sm font-semibold" style={{ color: '#000000' }}>Remix with AI</span>
                      </Button>
                      <Button
                        onClick={handleGetStarted}
                        className="flex items-center gap-2 rounded-lg px-5 py-3"
                        style={{ background: '#2E2E2E', boxShadow: '0px 2px 5px rgba(103, 110, 118, 0.08), 0px 0px 0px 1px rgba(103, 110, 118, 0.16), 0px 1px 1px rgba(0, 0, 0, 0.12)' }}
                      >
