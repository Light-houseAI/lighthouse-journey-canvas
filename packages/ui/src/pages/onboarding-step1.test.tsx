...
const mockSetSelectedInterest = vi.fn();
const mockLogoutAsync = vi.fn();
...
vi.mock('../stores/profile-review-store', () => ({
  useProfileReviewStore: () => ({
    setSelectedInterest: mockSetSelectedInterest,
  }),
}));
...
describe('OnboardingStep1', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render descriptions for each option', () => {
      ...
      expect(
        screen.getByText('Advance in my current field')
      ).toBeInTheDocument();
      expect(
  ...
  describe('Navigation', () => {
    it('should call logout when back to sign in is clicked', async () => {
      const user = userEvent.setup();
      mockLogoutAsync.mockResolvedValue(undefined);
