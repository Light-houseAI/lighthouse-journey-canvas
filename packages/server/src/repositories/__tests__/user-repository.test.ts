...
describe('User Repository Tests', () => {
  ...
  describe('Search Users', () => {
    ...
    it('should search users by last name with case-insensitive matching', async () => {
      // Arrange
      const mockUsers = [
        createTestUser({ firstName: 'Jane', lastName: 'Smith' } as any),
        createTestUser({ id: 2, firstName: 'Bob', lastName: 'Smithson' } as any),
      ];
      mockDb.__setSelectResult(mockUsers);
    ...
    it('should limit results to 20 users', async () => {
      // Arrange
      const mockUsers = Array.from({ length: 25 } as any, (_, i) =>
        createTestUser({
          id: i + 1,
          firstName: 'User',
          lastName: `${i + 1} as any`,
          email: `user${i + 1} as any@example.com`,
        })
      );
