...
import { nodePolicies, timelineNodes } from '@journey/schema';
...
export class NodePermissionRepository {
  ...
  async canAccess(
    ...
    try {
      ...
      this.logger.error('Error checking node access', Object.assign(error as any, {
        userId,
        nodeId,
        action,
        level,
  ...
  async getNodePolicies(nodeId: string): Promise<NodePolicy[]> {
    try {
      ...
      this.logger.error('Error getting node policies', Object.assign(error as any, {
        nodeId,
        error: error instanceof Error ? error.message : String(error),
      }));
      throw error;
    ...
    try {
      ...
      await this.database.transaction(async (tx) => {
            ...
            if (subjectKey === 'public') {
              await tx
                ...
                .where(
                  and(
                    ...
                    eq(nodePolicies.subjectType, 'public' as any)
                  )
                );
            } else {
              ...
              await tx
                ...
                .where(
                  and(
                    eq(nodePolicies.nodeId, nodeId),
                    eq(nodePolicies.subjectType, subjectType as any),
        ...
        if (policies.length > 0) {
          ...
          await tx.insert(nodePolicies).values(policyInserts as any);
        }
      });
      ...
      this.logger.error('Error setting node policies', Object.assign(error as any, {
        grantedBy,
  ...
  async deletePolicy(policyId: string, userId: number): Promise<void> {
    try {
      ...
      this.logger.error('Error deleting policy', Object.assign(error as any, {
        policyId,
        userId,
        error: error instanceof Error ? error.message : String(error),
      }));
  ...
  async updatePolicy(
    ...
    try {
      ...
      this.logger.error('Error updating policy', Object.assign(error as any, {
        policyId,
        userId,
        updates,
        error: error instanceof Error ? error.message : String(error),
  ...
  async isNodeOwner(userId: number, nodeId: string): Promise<boolean> {
    try {
      ...
      this.logger.error('Error checking node ownership', Object.assign(error as any, {
        userId,
        nodeId,
        error: error instanceof Error ? error.message : String(error),
      }));
