...
import type { ExperienceMatchesService } from '../services/experience-matches.service';
import { z } from 'zod';
import { BaseController } from './base-controller';

export interface ExperienceMatchesControllerDependencies {
  logger: Logger;
  experienceMatchesService: ExperienceMatchesService;
}
...
export class ExperienceMatchesController extends BaseController {
  ...
  private readonly experienceMatchesService: ExperienceMatchesService;
  ...
  async getMatches(req: Request, res: Response): Promise<void> {
    try {
      ...
      this.logger.error('Failed to get experience matches', { error });
   ...
   * (Optional endpoint for frontend to get query without full match data)
   */
  async getSearchQuery(req: Request, res: Response): Promise<void> {
    try {
      // Validate params
      const paramsResult = getMatchesParamsSchema.safeParse(req.params);
      if (!paramsResult.success) {
      ...
      // Get search query from service
      const searchQuery = await this.experienceMatchesService.getSearchQuery(nodeId, userId);
      ...
      if (searchQuery === null) {
        res.status(404).json({
          ...
          error: {
            code: 'NODE_NOT_FOUND',
            message: 'Node not found or not a current experience',
          },
        });
        return;
      }
      ...
      this.logger.error('Failed to get search query', { error });
