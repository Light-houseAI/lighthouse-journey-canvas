...
describe('PgVectorGraphRAGController', () => {
  ...
  describe('searchProfiles', () => {
    test('should handle valid request with query and limit', async () => {
      ...
      await controller.searchProfiles(mockReq);
    ...
    test('should throw ValidationError for missing query parameter', async () => {
      ...
      await expect(controller.searchProfiles(mockReq)).rejects.toThrow(ValidationError);
    });
    ...
    test('should throw error when service fails', async () => {
      ...
      await expect(controller.searchProfiles(mockReq)).rejects.toThrow('Service unavailable');
    });

    test('should throw ValidationError for limit exceeding maximum', async () => {
      const { ValidationError } = await import('@journey/schema');
      mockReq.body = {
        query: 'test query',
