...
export class ArangoDBGraphService {
  ...
  // ============================================================================
  // CONTEXT STITCHING OPERATIONS (Workstreams, Tool Mastery, Process Patterns)
  // ============================================================================

  /**
   * Upsert workstream node (Tier 1: Outcome-based groupings)
   * Follows existing ArangoDB patterns: withRetry wrapper, consistent key naming
   */
  async upsertWorkstream(data: {
    workstreamId: string;
    userId: number;
    name: string;
    ...
    const workstreamKey = `workstream_${data.userId}_${data.workstreamId}`;

    await this.withRetry(async () => {
      try {
        const query = aql`
          UPSERT { _key: ${workstreamKey} }
          INSERT {
            _key: ${workstreamKey},
            user_key: ${userKey},
            user_id: ${data.userId},
            ...
            name: ${data.name},
            outcome_description: ${data.outcomeDescription},
        ...
        await db.query(query);
        this.logger.debug('Upserted workstream', { workstreamKey });
      } catch (error) {
    ...
    }, `upsertWorkstream(${workstreamKey})`);
  }

  /**
   * Create SESSION_IN_WORKSTREAM edge
   * Uses existing createEdge pattern with retry logic
   */
  async createSessionInWorkstreamEdge(
    ...
    const sessionKey = `session_${sessionExternalId.replace(/[^a-zA-Z0-9]/g, '_')}`;
    const workstreamKey = `workstream_${userId}_${workstreamId}`;
  ...
  /**
   * Upsert tool mastery group (Tier 2: Tool usage patterns)
   * Key format: mastery_{userId}_{normalizedToolName}
   */
  async upsertToolMasteryGroup(data: {
    userId: number;
    toolName: string;
    ...
    const db = await this.ensureInitialized();
    const normalizedTool = data.toolName.toLowerCase().replace(/[^a-zA-Z0-9]/g, '_');
    const key = `mastery_${data.userId}_${normalizedTool}`;
    ...
    await this.withRetry(async () => {
      try {
        ...
        await db.query(query);
        this.logger.debug('Upserted tool mastery group', { key });
      } catch (error) {
        this.logger.error('Failed to upsert tool mastery group', {
          toolName: data.toolName,
          error: error instanceof Error ? error.message : String(error),
        });
        throw error;
      }
    }, `upsertToolMasteryGroup(${key})`);
  }

  /**
   * Upsert process pattern (Tier 3: Repetitive cross-tool sequences)
   * Key format: pattern_{userId}_{patternId}
   */
  async upsertProcessPattern(data: {
    patternId: string;
    userId: number;
    patternName: string;
    workflow: unknown[];
    ...
    const db = await this.ensureInitialized();
    const patternKey = `pattern_${data.userId}_${data.patternId}`;
      ...
      try {
        const query = aql`
          ...
          INSERT {
            ...
            avg_duration_seconds: ${data.avgDurationSeconds},
            first_seen: ${data.firstSeen},
            last_seen: ${data.lastSeen},
            automation_potential: ${data.automationPotential},
        ...
        await db.query(query);
        this.logger.debug('Upserted process pattern', { patternKey });
      } catch (error) {
        this.logger.error('Failed to upsert process pattern', {
          patternId: data.patternId,
          error: error instanceof Error ? error.message : String(error),
        });
        throw error;
      }
    }, `upsertProcessPattern(${patternKey})`);
  }

  /**
   * Get workstreams for a user (Tier 1)
   * Returns persisted workstream groupings with session edges
   */
      ...
      try {
        const query = aql`
          FOR ws IN workstreams
            FILTER ws.user_id == ${userId}
            FILTER ws.confidence >= ${minConfidence}
            SORT ws.last_activity DESC
            LIMIT ${limit}

            LET sessionIds = (
              FOR session, edge IN INBOUND ws._id SESSION_IN_WORKSTREAM
                RETURN session.external_id
            )

            RETURN MERGE(ws, { sessionIds })
        `;

        const cursor = await db.query(query);
        return await cursor.all();
      } catch (error) {
        this.logger.error('Failed to get workstreams by user', {
          userId,
          error: error instanceof Error ? error.message : String(error),
        });
        return [];
      }
    }, 'getWorkstreamsByUser');
  }

  /**
   * Get tool mastery groups for a user (Tier 2)
   * Includes session relationships
   */
  async getToolMasteryGroupsByUser(
    userId: number,
    toolName?: string
  ): Promise<unknown[]> {
    if (!this.enabled) {
    ...
    return this.withRetry(async () => {
      try {
        ...
        if (toolName) {
          const normalizedTool = toolName.toLowerCase().replace(/[^a-zA-Z0-9]/g, '_');
          const key = `mastery_${userId}_${normalizedTool}`;

          query = aql`
            FOR tm IN tool_mastery
              FILTER tm._key == ${key}

              LET sessionIds = (
                FOR session, edge IN OUTBOUND tm._id PATTERN_OBSERVED
                  RETURN session.external_id
              )

              RETURN MERGE(tm, { sessionIds })
          `;
        } else {
          query = aql`
            FOR tm IN tool_mastery
              FILTER tm.user_id == ${userId}
              SORT tm.total_time_seconds DESC
        ...
        this.logger.error('Failed to get tool mastery groups by user', {
          userId,
          toolName,
          error: error instanceof Error ? error.message : String(error),
        });
    ...
    }, 'getToolMasteryGroupsByUser');
  }

  /**
   * Get process patterns for a user (Tier 3)
   * Returns repetitive cross-tool workflow sequences
   */
      ...
      try {
        const query = aql`
          FOR pp IN process_patterns
            FILTER pp.user_id == ${userId}
            FILTER pp.frequency >= ${minFrequency}
            SORT pp.frequency DESC, pp.last_seen DESC
            LIMIT ${limit}

            LET sessionIds = (
              FOR session, edge IN OUTBOUND pp._id PATTERN_INSTANCE
                RETURN session.external_id
            )

            RETURN MERGE(pp, { sessionIds })
        `;

        const cursor = await db.query(query);
        return await cursor.all();
      } catch (error) {
        this.logger.error('Failed to get process patterns by user', {
          userId,
          error: error instanceof Error ? error.message : String(error),
        });
        return [];
      }
    }, 'getProcessPatternsByUser');
  }
