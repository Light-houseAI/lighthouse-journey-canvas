...
describe('useNodeBehaviors', () => {
  ...
  describe('Behavior Composition', () => {
    ...
    it('should calculate isActive correctly with different states', () => {
      ...
      let { result } = renderHook(() => useNodeBehaviors(mockNodeId));
      ...
      result = renderHook(() => useNodeBehaviors(mockNodeId)).result;
      expect(result.current.isActive).toBe(true);
  ...
  describe('Focus Behavior Integration', () => {
    it('should call focus store methods correctly', () => {
      ...
      expect(mockFocusStore.setFocusedExperience).toHaveBeenCalledWith(
        mockNodeId
      );
  ...
  describe('Selection Behavior Integration', () => {
    it('should call selection store methods correctly', () => {
      ...
      expect(mockSelectionStore.setSelectedNode).toHaveBeenCalledWith(
        mockNodeId
      );
  ...
  describe('Highlight Behavior Integration', () => {
    it('should call highlight store methods correctly', () => {
      ...
      expect(mockHighlightStore.setHighlightedNode).toHaveBeenCalledWith(
        mockNodeId,
        5000
      );
      ...
      expect(mockHighlightStore.highlightTemporary).toHaveBeenCalledWith(
        mockNodeId,
        2000
      );
      ...
      expect(mockHighlightStore.flashHighlight).toHaveBeenCalledWith(
        mockNodeId,
        3
      );
    });
  ...
  describe('Interaction Behavior Integration', () => {
    it('should call interaction store methods correctly', () => {
      ...
      expect(mockInteractionStore.setHoveredNode).toHaveBeenCalledWith(
        mockNodeId
      );
      ...
      expect(mockInteractionStore.setDraggedNode).toHaveBeenCalledWith(
        mockNodeId
      );
      ...
      expect(mockInteractionStore.setContextMenuNode).toHaveBeenCalledWith(
        mockNodeId
      );
  ...
  describe('Different Node IDs', () => {
    it('should work correctly with different node IDs', () => {
      ...
      expect(mockFocusStore.setFocusedExperience).toHaveBeenLastCalledWith(
        nodeId1
      );
