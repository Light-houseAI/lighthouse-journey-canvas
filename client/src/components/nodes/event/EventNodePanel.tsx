...
const EventView: React.FC<EventViewProps> = ({ node, onEdit, onDelete, canEdit, isDeleting }) => {
  const getEventTitle = () => {
    ...
    <>
      ...
      <div className="relative mb-6 p-6 rounded-2xl bg-gradient-to-br from-white to-slate-50 border border-slate-200/50 shadow-lg">
        ...
        <div className="relative">
          ...
          {node.meta.eventType && (
            <span className="inline-block mt-2 px-3 py-1 bg-orange-100 text-orange-700 text-sm rounded-full capitalize font-medium">
              {node.meta.eventType}
            </span>
          )}
          {node.meta.location && (
      ...
      {(node.meta.startDate || node.meta.endDate) && (
        <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-orange-50 to-slate-50 border border-orange-200/50">
      ...
      {/* Organizer */}
      {node.meta.organizer && (
        <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-orange-50 to-white border border-orange-200/50">
          <span className="text-xs font-semibold text-orange-600 uppercase tracking-wider">Organizer</span>
          <p className="text-slate-900 mt-2 font-medium">{node.meta.organizer}</p>
        </div>
      )}
      ...
      {canEdit && (
        <div className="flex gap-3 mt-8">
          <button
            onClick={onEdit}
            className="group relative flex-1 px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white font-medium transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/25 overflow-hidden"
          >
...
export const EventNodePanel: React.FC<EventNodePanelProps> = ({ node, deleteNode: deleteNodeProp }) => {
