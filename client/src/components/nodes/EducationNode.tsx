...
import { useChatIntegration, createEducationUpdateMessage } from '@/hooks/useChatIntegration';
import { 
  EducationData, 
  getTypeIcon, 
  getTypeGradient, 
...
const EducationNode: React.FC<NodeProps> = ({ data, selected, id }) => {
  ...
  const gradient = getTypeGradient(educationData.type);
  const icon = getTypeIcon(educationData.type);
  ...
  const [editTitle, setEditTitle] = useState(educationData.title);
  ...
  const handleClick = (event: React.MouseEvent) => {
    ...
    // Education nodes have simple click behavior
    if ((data as any).onNodeClick && typeof (data as any).onNodeClick === 'function') {
      (data as any).onNodeClick(educationData, id);
    }
  };
  ...
  const handleSave = async (event: React.MouseEvent) => {
    ...
    try {
      ...
      // Update local data
      educationData.title = editTitle;
      educationData.description = editDescription;
  ...
  const handleCancel = (event: React.MouseEvent) => {
    event.stopPropagation();
    setEditTitle(educationData.title);
  ...
  const handleAddUpdate = (event: React.MouseEvent) => {
    ...
    // Create education-specific update message and open chat
    const message = createEducationUpdateMessage(
      educationData.title,
    ...
    <div onClick={handleClick} className={`relative ${getBlurClasses(isBlurred, isFocused)}`}>
      ...
      <div className={`absolute left-1/2 transform -translate-x-1/2 z-10 ${
        ...
        <div className={`bg-gray-900/90 backdrop-blur-sm rounded-2xl px-4 py-3 shadow-xl text-center border ${
          ...
          {isEditing ? (
            ...
            <>
              ...
              <p className={`text-xs mb-2 ${isSubMilestone ? 'text-yellow-200/80' : 'text-white/80'}`}>
                {formatDate(educationData.date)}
              </p>
              <p className={`text-xs mb-2 text-white/80`}>
                {educationData.duration}
              </p>
              {(isSubMilestone || educationData.type === 'update') && (
          ...
          {educationData.organization && !isSubMilestone && (
          ...
          {hasSubMilestones && !isSubMilestone && (
            <div className="flex items-center justify-center gap-1 mt-2">
              ...
              <span className="text-blue-300 text-xs">Has achievements</span>
            </div>
          )}
        </div>
