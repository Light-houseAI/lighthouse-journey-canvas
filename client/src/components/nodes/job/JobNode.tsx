/**
 * JobNode - Dedicated Job Node Component
 *
 * Specialized node component for job experiences with optimized layout
 * and job-specific information display.
 */
...
// Props passed to JobNode by React Flow
...
export const JobNode: React.FC<NodeProps<JobNodeData>> = ({
  data,
}) => {
  const {
    node,
  ...
  // Job-specific node styling with professional theme
  const getJobNodeStyle = (): React.CSSProperties => {
    const baseColor = '#0891b2'; // Deep cyan for professional look
    const accentColor = '#0e7490'; // Even deeper cyan
    const isHighlighted = isSelected || isFocused;

    return {
      // Professional gradient with job-specific colors
      ...
      borderRadius: '20px',
      padding: '0px',
      width: '240px', // Slightly wider for job info
      height: '180px', // Taller for company + role info
  ...
  return (
    <div
      ref={nodeRef}
      style={getJobNodeStyle()}
      onClick={handleNodeClick}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
      className="job-node group relative transform hover:scale-[1.02] transition-transform duration-300"
    >
      ...
      <Handle
        ...
        id="left"
        style={{ background: '#0891b2', border: 'none', width: 8, height: 8 }}
      />
      ...
      {/* Job-specific layout: Icon+Label | Company | Role | Location | Actions */}
        ...
        >
          ...
          >
            ...
            <div className="relative z-10 h-full w-full flex flex-col justify-between px-4">
              ...
              {/* Row 2: Enhanced Job Information */}
