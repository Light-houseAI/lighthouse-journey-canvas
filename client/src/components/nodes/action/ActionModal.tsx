...
export const ActionModal: React.FC<ActionModalProps> = ({
  ...
  const form = useForm<ActionFormData>({
    ...
    defaultValues: {
      title: 'Action Title', // Required by actionMetaSchema
      description: '',
      startDate: '',
      endDate: '',
  ...
  useEffect(() => {
    if (context.suggestedData) {
      Object.entries(context.suggestedData).forEach(([key, value]) => {
        form.setValue(key as keyof ActionFormData, value as any);
      });
    }
  ...
  const getContextDescription = () => {
    switch (context.insertionPoint) {
      ...
      case 'branch':
        return (
          <span>
            Adding action to <strong>{context.parentNode?.title}</strong>
          </span>
        );
      default:
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      />
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-gradient-to-br from-white via-orange-50 to-amber-50/30 border border-orange-200/50 shadow-2xl backdrop-blur-sm">
        ...
        <div className="relative z-10">
          <DialogHeader className="pb-6 border-b border-orange-200/50">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <DialogTitle id="modal-title" className="text-2xl font-bold bg-gradient-to-r from-orange-800 to-amber-600 bg-clip-text text-transparent">
                  Add Action
                </DialogTitle>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6 add-node-form pt-6">
              ...
              <div className="flex justify-end space-x-4 pt-8 border-t border-orange-200/50 mt-8">
                <button
                  type="button"
                  onClick={onClose}
