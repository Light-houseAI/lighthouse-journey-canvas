...
import { actionFormSchema, ActionFormData } from './schema';
  ...
  return (
    <>
      ...
      <div className="relative mb-6 p-6 rounded-2xl bg-gradient-to-br from-white to-orange-50 border border-orange-200/50 shadow-lg">
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-orange-500/5 via-transparent to-amber-500/5"></div>
        <div className="relative">
          ...
          {(node.meta as any).category && (
            <span className="inline-block mt-3 px-4 py-2 bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 text-sm rounded-full font-medium capitalize border border-orange-200/50">
              {(node.meta as any).category}
            </span>
          )}
        </div>
      ...
      {(node.meta.startDate || node.meta.endDate) && (
        <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200/50">
          <span className="text-xs font-semibold text-orange-600 uppercase tracking-wider">Duration</span>
      ...
      <div className="flex gap-3 mt-8">
        <button
          onClick={onEdit}
          className="group relative flex-1 px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-amber-600 text-white font-medium transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/25 overflow-hidden"
        >
...
const ActionEdit: React.FC<ActionEditProps> = ({ node, onSave, onCancel, loading }) => {
  ...
  return (
    <>
      ...
      <div className="relative mb-8 p-6 rounded-2xl bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200/50">
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-orange-500/5 via-transparent to-amber-500/5"></div>
        <div className="relative">
          <h3 className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
            Edit Action
          </h3>
          <p className="text-slate-600 mt-2">Update your action information</p>
          <div className="w-16 h-1 bg-gradient-to-r from-orange-500 to-amber-600 rounded-full mt-3"></div>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-sm font-semibold text-slate-700">Action Title</FormLabel>
                  <FormControl>
                    <div className="relative">
                      <Input 
                        placeholder="Action title" 
                        className="border-slate-300 focus:border-orange-500 focus:ring-orange-500/20 rounded-lg bg-white/50 backdrop-blur-sm transition-all duration-200"
                        {...field} 
                      />
...
export const ActionNodePanel: React.FC<ActionNodePanelProps> = ({ node }) => {
  ...
  const handleSave = async (data: { meta: Partial<NodeMetadata> }) => {
    try {
      ...
      console.error('Failed to save action node:', error);
    }
  };
      ...
      >
        ...
        <div className="relative h-full bg-gradient-to-br from-orange-50 via-white to-amber-50 shadow-2xl border border-orange-200">
          ...
          <div className="absolute inset-0 rounded-none overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-orange-400 to-transparent animate-shimmer"></div>
          </div>
            ...
            <div className="px-6 py-4 border-b border-orange-200/50 flex items-center justify-between bg-gradient-to-r from-orange-50/50 to-white/50 backdrop-blur-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="w-8 h-0.5 bg-gradient-to-r from-orange-500 to-amber-600 rounded-full"></div>
                </div>
              </div>
              <button
                onClick={handleClose}
