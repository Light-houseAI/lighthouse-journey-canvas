...
// Generic component for simple node types (update, event, skill, transition)
const GenericNode: React.FC<NodeProps> = ({ data, selected, id }) => {
  const nodeData = data as any;
  const gradient = getTypeGradient(nodeData.type);
  const icon = getTypeIcon(nodeData.type);
  
  // Extended data from the actual node
  const isUpdated = nodeData.isUpdated;
  const isSubMilestone = nodeData.isSubMilestone;
  const hasSubMilestones = nodeData.hasSubMilestones;
  const onAddSubMilestone = nodeData.onAddSubMilestone;
  const isFocused = nodeData.isFocused;
  const isBlurred = nodeData.isBlurred;
  ...
  const handleClick = (event: React.MouseEvent) => {
    ...
    if (nodeData.onNodeClick && typeof nodeData.onNodeClick === 'function') {
  ...
  const handleDelete = async (event: React.MouseEvent) => {
    ...
    if (confirm('Are you sure you want to delete this item?')) {
      try {
        ...
        if (nodeData.onNodeDelete && typeof nodeData.onNodeDelete === 'function') {
        ...
        <div className={`bg-gray-900/90 backdrop-blur-sm rounded-2xl px-4 py-3 shadow-xl text-center border ${
          ...
          {isEditing ? (
            ...
            <>
              ...
              {(isSubMilestone || nodeData.type === 'update') && (
          ...
          {nodeData.organization && !isSubMilestone && (
          ...
          {hasSubMilestones && !isSubMilestone && (
            <div className="flex items-center justify-center gap-1 mt-2">
              ...
              <span className="text-gray-300 text-xs">Has items</span>
            </div>
          )}
        </div>
...
// NodeFactory component that routes to the appropriate node type
const NodeFactory: React.FC<NodeProps> = (props) => {
  const nodeType = (props.data as any).type as NodeType;

  switch (nodeType) {
    case 'job':
      return <ExperienceNode {...props} />;
    case 'project':
      return <ProjectNode {...props} />;
    case 'education':
      return <EducationNode {...props} />;
    case 'update':
    case 'event':
    case 'skill':
    case 'transition':
