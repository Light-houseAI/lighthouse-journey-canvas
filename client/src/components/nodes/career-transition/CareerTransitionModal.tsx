...
import { careerTransitionFormSchema, CareerTransitionFormData } from './schema';
...
interface NodeContext {
  ...
  suggestedData?: Partial<CareerTransitionFormData>;
}

interface CareerTransitionModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const CareerTransitionModal: React.FC<CareerTransitionModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  context,
  ...
  const form = useForm<CareerTransitionFormData>({
    resolver: zodResolver(careerTransitionFormSchema),
    defaultValues: {
      title: 'Career Transition Title', // Required by careerTransitionMetaSchema
      description: '',
      startDate: '',
      endDate: '',
  ...
  useEffect(() => {
    if (context.suggestedData) {
      Object.entries(context.suggestedData).forEach(([key, value]) => {
        form.setValue(key as keyof CareerTransitionFormData, value as any);
      });
    }
  }, [context.suggestedData, form]);

  const handleFormSubmit = async (data: CareerTransitionFormData) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to save career transition');
    } finally {
  ...
  const getContextDescription = () => {
    switch (context.insertionPoint) {
      ...
      case 'branch':
        return (
          <span>
            Adding career transition to <strong>{context.parentNode?.title}</strong>
          </span>
        );
      default:
        return 'Adding new career transition';
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      />
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-gradient-to-br from-white via-violet-50 to-purple-50/30 border border-slate-200/50 shadow-2xl backdrop-blur-sm">
        {/* Subtle Background Effects */}
        <div className="absolute inset-0 bg-gradient-to-r from-violet-500/5 via-transparent to-purple-500/5 rounded-lg"></div>
          ...
          <DialogHeader className="pb-6 border-b border-slate-200/50">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <div className="w-16 h-1 bg-gradient-to-r from-violet-500 to-purple-600 rounded-full mt-2"></div>
              </div>
            </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6 add-node-form pt-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-sm font-semibold text-slate-700">Career Transition Title</FormLabel>
                    <FormControl>
                      <div className="relative group">
                        <Input
                          placeholder="Career transition title"
                          className="border-slate-300 focus:border-violet-500 focus:ring-violet-500/20 rounded-lg bg-white/50 backdrop-blur-sm transition-all duration-200 group-hover:bg-white/70"
                          {...field}
                        />
                        <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-violet-500/5 to-purple-500/5 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="relative group">
                        <Textarea
                          placeholder="Career transition description"
                          rows={3}
                          className="border-slate-300 focus:border-violet-500 focus:ring-violet-500/20 rounded-lg bg-white/50 backdrop-blur-sm transition-all duration-200 group-hover:bg-white/70"
                          {...field}
                        />
              ...
              <div className="flex justify-end space-x-4 pt-8 border-t border-slate-200/50 mt-8">
                ...
                <button
                  ...
                  className="group relative px-6 py-3 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white font-medium transition-all duration-300 hover:shadow-lg hover:shadow-violet-500/25 overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
                >
