/**
 * ProjectNode - Dedicated Project Node Component
 *
 * Specialized node component for project experiences with optimized layout
 * and project-specific information display.
 */
...
// Props passed to ProjectNode by React Flow
...
export const ProjectNode: React.FC<NodeProps<ProjectNodeData>> = ({
  ...
  // Project-specific node styling with creative theme
  const getProjectNodeStyle = (): React.CSSProperties => {
    const baseColor = '#7c3aed'; // Deep purple for creativity and innovation
    const accentColor = '#6d28d9'; // Even deeper purple
    const isHighlighted = isSelected || isFocused;

    return {
      // Creative gradient with project-specific colors
      ...
      borderRadius: '20px',
      padding: '0px',
      width: '240px', // Slightly wider for project info
      height: '180px', // Taller for project title + type info
  ...
  const title = node.meta?.title || node.title || 'Project';
  const projectType = node.meta?.projectType || '';
  ...
  return (
    <div
      ref={nodeRef}
      style={getProjectNodeStyle()}
      onClick={handleNodeClick}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
      className="project-node group relative transform hover:scale-[1.02] transition-transform duration-300"
    >
      ...
      <Handle
        ...
        id="left"
        style={{ background: '#7c3aed', border: 'none', width: 8, height: 8 }}
      />
      ...
      {/* Project-specific layout: Icon+Label | Title | Type | Status | Actions */}
      ...
      {/* Project-specific Hover Tooltip */}
        ...
        >
          ...
          >
            ...
            <div className="relative z-10 h-full w-full flex flex-col justify-between px-4">
              ...
              {/* Row 2: Enhanced Project Information */}
