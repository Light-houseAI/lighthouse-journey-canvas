...
export const ProjectModal: React.FC<ProjectModalProps> = ({
  ...
  const form = useForm<ProjectFormData>({
    ...
    defaultValues: {
      title: 'Project Title', // Required by projectMetaSchema
      description: '',
      technologies: [],
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      />
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-gradient-to-br from-white via-purple-50 to-pink-50/30 border border-purple-200/50 shadow-2xl backdrop-blur-sm">
        {/* Subtle Background Effects */}
        <div className="absolute inset-0 bg-gradient-to-r from-purple-500/5 via-transparent to-pink-500/5 rounded-lg"></div>
        <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-purple-400/50 to-transparent"></div>
        
        <div className="relative z-10">
          <DialogHeader className="pb-6 border-b border-purple-200/50">
            {/* Enhanced Header */}
            <div className="flex items-center gap-4 mb-4">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center shadow-lg">
                <NodeIcon type="project" size={24} className="text-white" />
              </div>
              <div>
                <DialogTitle id="modal-title" className="text-2xl font-bold bg-gradient-to-r from-purple-800 to-pink-600 bg-clip-text text-transparent">
                  Add Project
                </DialogTitle>
                <div className="w-16 h-1 bg-gradient-to-r from-purple-500 to-pink-600 rounded-full mt-2"></div>
              </div>
            </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6 add-node-form pt-6">
                    ...
                    <FormControl>
                      <div className="relative group">
                        <Input
                          ...
                          className="border-slate-300 focus:border-purple-500 focus:ring-purple-500/20 rounded-lg bg-white/50 backdrop-blur-sm transition-all duration-200 group-hover:bg-white/70"
                          {...field}
                        />
                        <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-purple-500/5 to-pink-500/5 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <div className="relative group">
                            <SelectTrigger className="border-slate-300 focus:border-purple-500 focus:ring-purple-500/20 rounded-lg bg-white/50 backdrop-blur-sm transition-all duration-200 group-hover:bg-white/70">
              ...
              <div className="flex justify-end space-x-4 pt-8 border-t border-purple-200/50 mt-8">
                <button
                  type="button"
                  onClick={onClose}
                ...
                <button
                  ...
                  className="group relative px-6 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-600 text-white font-medium transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/25 overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
                >
