...
import { NodePolicyCreateDTO } from '@shared/types';
...
export const useShareStore = create<ShareState>()(
  devtools(
    immer((set, get) => ({
      ...
      // Modal actions
      openModal: (nodes?: TimelineNode[]) => {
        console.log('🔥 openModal called with nodes:', nodes?.length || 0);
        set((state) => {
      ...
      openModalWithSelection: (
        ...
        console.log('🔥 openModalWithSelection called with:', {
        ...
        set((state) => {
          ...
          console.log(
            '✅ Modal with selection state updated, isModalOpen:',
            state.isModalOpen
          );
        });
      },
      ...
      executeShare: async () => {
        ...
        try {
          const { setBulkNodePermissions, getBulkNodePermissions } =
            await import('../services/permission-api');
      ...
      fetchCurrentPermissions: async (nodeIds: string[]) => {
        ...
        try {
          const { getBulkNodePermissions } = await import(
          ...
          console.log(
            '📋 Found bulk permissions by node:',
      ...
      removePermission: async (subjectKey: string, nodeId?: string) => {
        try {
          const { deleteNodePermission } = await import(
            '../services/permission-api'
          );
          ...
          if (subjectKey.startsWith('user-')) {
            ...
            const userPerm = currentPermissions.users.find(
              (u) => u.id === userId
            );
            if (userPerm) {
              if (nodeId) {
          ...
          // Check public permissions - subjectKey format: "public"
            ...
            if (nodeId) {
                ...
                policiesToRemove = [
                  {
                    nodeId: nodeId,
                    policyId:
                      publicPerm.policyIds[nodeIndex] ||
                      publicPerm.policyIds[0],
                  },
                ];
              }
            } else {
              ...
              policiesToRemove = publicPerm.nodes.map((node, index) => ({
                nodeId: node.nodeId,
                policyId:
                  publicPerm.policyIds[index] || publicPerm.policyIds[0],
              }));
            }
          }
          ...
          set((draft) => {
            if (subjectKey.startsWith('user-')) {
              const userId = parseInt(subjectKey.split('-')[1]);
              const userIndex = draft.currentPermissions.users.findIndex(
                (u) => u.id === userId
              );
              if (userIndex >= 0) {
                if (nodeId) {
                  // Remove specific node from user's permission
                  const nodeIndex = draft.currentPermissions.users[
                    userIndex
                  ].nodes.findIndex((n) => n.nodeId === nodeId);
                  if (nodeIndex >= 0) {
                    draft.currentPermissions.users[userIndex].nodes.splice(
                      nodeIndex,
                      1
                    );
                    draft.currentPermissions.users[userIndex].policyIds.splice(
                      nodeIndex,
                      1
                    );
                  }
                  ...
                  if (
                    draft.currentPermissions.users[userIndex].nodes.length === 0
                  ) {
              ...
              const orgId = parseInt(subjectKey.split('-')[1]);
              const orgIndex = draft.currentPermissions.organizations.findIndex(
                (o) => o.id === orgId
              );
              if (orgIndex >= 0) {
                if (nodeId) {
                  // Remove specific node from org's permission
                  const nodeIndex = draft.currentPermissions.organizations[
                    orgIndex
                  ].nodes.findIndex((n) => n.nodeId === nodeId);
                  if (nodeIndex >= 0) {
                    draft.currentPermissions.organizations[
                      orgIndex
                    ].nodes.splice(nodeIndex, 1);
                    draft.currentPermissions.organizations[
                      orgIndex
                    ].policyIds.splice(nodeIndex, 1);
                  }
                  // If no nodes left, remove the entire org permission
                  if (
                    draft.currentPermissions.organizations[orgIndex].nodes
                      .length === 0
                  ) {
              ...
              subjectKey === 'public' &&
              draft.currentPermissions.public
            ) {
              if (nodeId) {
                // Remove specific node from public permission
                const nodeIndex =
                  draft.currentPermissions.public.nodes.findIndex(
                    (n) => n.nodeId === nodeId
                  );
                if (nodeIndex >= 0) {
                  draft.currentPermissions.public.nodes.splice(nodeIndex, 1);
                  draft.currentPermissions.public.policyIds.splice(
                    nodeIndex,
                    1
                  );
                }
      ...
      updatePermission: async (
        ...
        try {
          ...
          if (policiesToUpdate.length === 1) {
            ...
            const { updateNodePermission } = await import(
              '../services/permission-api'
            );
            ...
            const { updateBulkNodePermissions } = await import(
              '../services/permission-api'
            );
            ...
            if (subjectKey.startsWith('user-')) {
              ...
              if (userIndex >= 0) {
                if (nodeId) {
                  ...
                  draft.currentPermissions.users[userIndex].accessLevel =
                    newLevel;
                }
              }
            } else if (subjectKey.startsWith('org-')) {
              ...
              if (orgIndex >= 0) {
                if (nodeId) {
                  ...
                  // Multi-node operation - update the subject's overall access level
                  draft.currentPermissions.organizations[orgIndex].accessLevel =
                    newLevel;
                }
              }
            } else if (
