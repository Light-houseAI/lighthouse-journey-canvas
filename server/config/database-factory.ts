...
export class DatabaseFactory {
  ...
  static async createConfig(
    ...
    const environment =
      options.environment || DatabaseFactory.detectEnvironment();
   ...
   */
  private static async createTestConfig(
    options: DatabaseFactoryOptions
    ...
    const localTestConnectionString =
  ...
  private static detectEnvironment(): DatabaseEnvironment {
    ...
    // Check if explicitly running in test context (more specific checks)
    if (
      process.env.VITEST === 'true' ||
      process.env.NODE_ENV === 'test' ||
      (process.argv.length > 1 && process.argv[1].includes('vitest'))
    ) {
      return 'test';
    }
