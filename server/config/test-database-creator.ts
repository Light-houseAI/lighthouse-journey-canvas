...
export class TestDatabaseCreator {
   ...
   */
  static async createTestDatabaseFromTemplate(testDatabaseName: string): Promise<void> {
    ...
    try {
      ...
      const localConnectionString = process.env.TEST_DATABASE_URL || 'postgresql://test_user:test_password@localhost:5433/lighthouse_test';
      ...
      try {
        ...
        // Create mastra_ai schema
        await testPool.query('CREATE SCHEMA IF NOT EXISTS mastra_ai');
        
        // Create tables with correct schema matching shared/schema.ts
        ...
        await testPool.query(`
          ...
          VALUES (999, 'test-user@example.com', '$2b$10$test.hash.for.test.user.only', 'grow-career', true, '2025-07-30T20:40:16.433Z'::timestamp) 
        ...
        `, [
          JSON.stringify({
            ...
            "about": "Results-oriented Product Manager with over 10 years of experience...",
            "experiences": [
              {
                "title": {"name": "senior product manager - tech", "class": "research_and_development", "role": "product", "sub_role": "product_management", "levels": ["senior"]},
                "company": "amazon", "description": "", "start": "Mar 2025", "end": "Present"
              },
              {
                "title": {"name": "senior product manager", "class": "research_and_development", "role": "product", "sub_role": "product_management", "levels": ["senior"]},
                "company": "walmart", "description": "", "start": "Apr 2024", "end": "Mar 2025"
              }
            ],
            "education": [{"school": "Carnegie Mellon University", "degree": "", "field": "", "start": "", "end": ""}],
            "skills": ["agile methodologies", "javascript", "product management"]
          }),
          JSON.stringify({
            "name": "sonam mishra",
            "headline": "senior product manager - tech",
          ...
          JSON.stringify([
            {
              ...
              "type": "subtask",
              "date": "2025-07-24",
              "description": "Building a PoC for allowing internal users to share product improvements",
              "skills": [],
              "organization": "amazon",
              "isSubMilestone": true
            }
        ...
        console.log(`✅ Created test database with fixture data: ${testDatabaseName}`);
      ...
      if (error.code === '42P04') {
        ...
        console.error(`❌ Failed to create test database ${testDatabaseName}:`, error);
        throw error;
      }
    }
  }
  ...
  static async createTestDatabase(testDatabaseName: string): Promise<void> {
    ...
    try {
      ...
      const testConnectionString = process.env.DATABASE_URL?.replace(/\/[^/]*$/, `/${testDatabaseName}`);
      ...
      if (testConnectionString.includes('sslmode=require') || 
  ...
  static async dropTestDatabase(testDatabaseName: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`⚠️ Error dropping test database ${testDatabaseName}:`, error);
  ...
  static async cleanupAllTestDatabases(): Promise<void> {
    console.log(`🧹 Cleaning up ${TestDatabaseCreator.createdDatabases.size} test databases...`);
    
    const cleanupPromises = Array.from(TestDatabaseCreator.createdDatabases).map(dbName =>
      TestDatabaseCreator.dropTestDatabase(dbName)
    );
  ...
  private static async getAdminPool(): Promise<Pool> {
    if (!TestDatabaseCreator.adminPool) {
      ...
      if (adminConnectionString.includes('localhost:5433')) {
        ...
        sslConfig = false;
      } else if (adminConnectionString.includes('sslmode=require') || 
