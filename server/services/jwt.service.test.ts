...
describe('JWTService', () => {
  ...
  describe('verifyAccessToken', () => {
    ...
    it('should throw error for expired token', () => {
      ...
      const expiredToken = jwt.sign(
        ...
        'test-access-secret',
        { expiresIn: '0s', issuer: 'lighthouse-app', audience: 'lighthouse-users' }
      );
    ...
    it('should throw error for token with wrong issuer', () => {
      const wrongIssuerToken = jwt.sign(
        { userId: mockUser.id, email: mockUser.email },
        'test-access-secret',
        { expiresIn: '15m', issuer: 'wrong-issuer', audience: 'lighthouse-users' }
      );
    ...
    it('should throw error for token with wrong audience', () => {
      const wrongAudienceToken = jwt.sign(
        { userId: mockUser.id, email: mockUser.email },
        'test-access-secret',
        { expiresIn: '15m', issuer: 'lighthouse-app', audience: 'wrong-audience' }
      );
  ...
  describe('verifyRefreshToken', () => {
    ...
    it('should throw error for expired refresh token', () => {
      const expiredToken = jwt.sign(
        { userId: mockUser.id, tokenId: 'test-id' },
        'test-refresh-secret',
        { expiresIn: '0s', issuer: 'lighthouse-app', audience: 'lighthouse-refresh' }
      );
  ...
  describe('isTokenExpired', () => {
    ...
    it('should return true for expired token', async () => {
      const expiredToken = jwt.sign(
        { userId: mockUser.id, exp: Math.floor(Date.now() / 1000) - 1 }, // 1 second ago
        'test-access-secret'
      );
  ...
  describe('error handling', () => {
    ...
    it('should handle tokens signed with wrong secret', () => {
      const tokenWithWrongSecret = jwt.sign(
        { userId: mockUser.id, email: mockUser.email },
        'wrong-secret',
        { expiresIn: '15m', issuer: 'lighthouse-app', audience: 'lighthouse-users' }
      );
