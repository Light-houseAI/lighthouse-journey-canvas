...
export const export const errorHandlerMiddleware = (err: any, req: Request, res: Response, _next: NextFunction) => {
  ...
  console.error('🚨 Global Error Handler - Complete Error Details:', {
    ...
    body: typeof req.body === 'object' ? 
      JSON.stringify(req.body).substring(0, 1000) + (JSON.stringify(req.body).length > 1000 ? '...[truncated]' : '') : 
      req.body,
    stack: err.stack?.substring(0, 2000) + (err.stack?.length > 2000 ? '...[truncated]' : '')
  });
  ...
  if (status < 500) {
    ...
    } else if (err.message?.includes('API key') || err.message?.includes('OpenAI')) {
