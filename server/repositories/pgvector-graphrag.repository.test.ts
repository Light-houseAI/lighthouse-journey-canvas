...
describe('PgVectorGraphRAGRepository', () => {
  ...
  describe('vectorSearch', () => {
    ...
    test('should handle empty results', async () => {
      ...
      const result = await repository.vectorSearch(embedding, { limit: 10 });
  ...
  describe('createChunk', () => {
    test('should create a new chunk with embedding', async () => {
      ...
      expect(result.chunkText).toBe(chunkData.chunkText);
    });
  });
  ...
  describe('createEdge', () => {
    test('should create an edge between chunks', async () => {
      ...
      mockPool.query.mockResolvedValue({ rows: [mockEdge] });

      const result = await repository.createEdge(edgeData);

      expect(result.id).toBe('edge-1');
      expect(result.relType).toBe('parent_child');
    });
  });
